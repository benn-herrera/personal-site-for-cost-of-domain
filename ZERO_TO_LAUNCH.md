...
* Install or upgrade to the latest LTS version - wrangler requires version 20+ (as of this writing).
...
* *macOS only*: site-tool binary needs to be made runnable - two options:
  * low-trust/safe (recommended):
    * Go language setup is a single step. 
    * ```brew install go && make -C maint/site-tool```
  * high trust/convenient: ```xattr -d com.apple.quarantine maint/bin/site-tool-drawin-*``` 
...
If you skipped the domain purchase step and are not sure what your domain name will be you can come back to this later.
...
* Edit `wrangler.jsonc` and change the "name" field to match your repo name
  ...
  * Having the Worker name match the repo is not required but it keeps things clear when they do
...
* If you're going to serve multiple domains
  ...
  * for any beyond two, copy `my-personal-site.me` under `public/` to a new directory for each domain you want to serve
  * INFO (nothing to do here): The `sites` list in `src/index.js` is maintained automatically — running `make` from any site directory updates it to match the current directories in `public/`. No manual edits needed.
...
**Now ask your coding agent to finish the rename.** Give it the actual values for your setup and ask it to make the following changes:

* `maint/common.mk` — update `SITE_AUTHOR` to your name (e.g. `First M. Last`)
...
## Create a Cloudflare Worker project and attach your repo
...
## Closing Note
* If you make a change you think others would benefit from by all means fork the original GitHub repo and submit a pull request!
